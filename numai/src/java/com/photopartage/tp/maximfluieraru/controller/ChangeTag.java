 ...
 * @author Maxim
 */
@WebServlet(name = "ChangeTag", urlPatterns = {"/ChangeTag"})
public class ChangeTag extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int idP = Integer.parseInt(idPhoto);
        String tag = request.getParameter("btn");
        ...
        photo.setPhoto_id(idP);
        for (Photo p : user.getUser_photos()) {
            if (p.equals(photo)) {
                p.setTag(tag);
                photo = p;
            }
